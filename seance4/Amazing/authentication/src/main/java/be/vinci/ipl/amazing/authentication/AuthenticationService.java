...
import be.vinci.ipl.amazing.authentication.models.InsecureCredentials;
...
public class AuthenticationService {
    ...
    public boolean createOne(InsecureCredentials credentials) {
        if (repository.existsById(credentials.getPseudo())) return false;
     ...
     * @param pseudo Pseudo of the user credentials
    ...
    public String connect(InsecureCredentials insecureCredentials) {
        ...
        return JWT.create().withIssuer("auth0").withClaim("pseudo", credentials.getPseudo()).sign(this.jwtAlgorithm);
    }
